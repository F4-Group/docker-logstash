...
#install logstash
RUN wget -q https://download.elastic.co/logstash/logstash/logstash-1.5.5.tar.gz -O /tmp/logstash.tar.gz && \
    (cd /tmp && tar zxf logstash.tar.gz && mv logstash-1.5.5 /opt/logstash && \
    rm logstash.tar.gz)
